...
pub fn draw_char_box(Pos(x, y): Pos, c: u8, color: Color, canvas: &mut Canvas) {
    let x = x as i32 * CS as i32;
    let y = y as i32 * CS as i32;
    // background
...
pub fn draw_highlight_box(Pos(x, y): Pos, w: usize, h: usize, color: Color, canvas: &mut Canvas) {
    canvas.set_draw_color(color);
    let x = x as i32 * CS as i32;
    let y = y as i32 * CS as i32;
    if h == 0 {
...
pub fn draw_highlight(p: Pos, color: Color, canvas: &mut Canvas) {
    draw_highlight_box(p, 1, 1, color, canvas);
}

pub fn draw_string(Pos(x, y): Pos, s: &[u8], color: impl Fn(usize) -> Color, canvas: &mut Canvas) {
